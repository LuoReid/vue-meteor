<template>
  <div class="production-dashboard">
    ...
    <section class="indicators">
      <productionIndicator
        v-bind:value="average / 100"
        title="Average"
        v-bind:info="Math.round(average)"
      />
      <ProductionIndicator
        class="danger"
        v-bind:value="errorRate"
        title="Errors"
        v-bind:info="`${Math.round(errorRate * 100)} %`"
      />
    </section>
    <section class="list">
      <div v-for="item of measures" v-bind:key="item._id">
...
export default {
  components: { ProductionIndicator },
  data() {
    return {
      measures: []
    };
